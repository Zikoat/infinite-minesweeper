...
export default class SimpleBot {
	...
	openToQueue() {
		this.field.getAll()
			...
			.forEach(cell=>{
				...
				const neighboringClosedCellsAmount = neighboringCells.filter(cell=>cell.isOpen).length; // type int
				const neighboringFlagAmount = neighboringCells.filter(cell=>cell.isFlagged).length; // type int

				let restNeighbors = neighboringCells
					.filter(neighbor=>!(neighbor.isFlagged || neighbor.isOpen));
	...
	runBotSimple() {
		let steps = 0;
		let prevOpened = -1;
		while(this.field.getAll().filter(cell=>cell.isOpen).length!==prevOpened){
			steps++;
